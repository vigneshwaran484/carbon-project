/**
 * Calculate carbon credits using scientifically validated formula.
 *
 * Constants:
...
function calculateCarbonCredits(area, biomassFactor, survivalRate) {
    // Step 1: Biomass = Area × BiomassFactor
    const biomass = area * biomassFactor;

    // Step 2: Carbon = Biomass × 0.47
    const carbon = biomass * 0.47;

    // Step 3: CO₂ Equivalent = Carbon × 3.67
    const co2 = carbon * 3.67;

    // Step 4: Survival Rate Adjustment
    const adjustedCo2 = co2 * (survivalRate / 100);

    // Step 5: Buffer Deduction (15%)
    const finalCo2 = adjustedCo2 * 0.85;

    // Step 6: Credits Issued
